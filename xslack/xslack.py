                    ...
                    for client in clients:
                        text = re.sub(r"(?:^| |\n)@(?:\d|[a-z]){1,23}(?:$| |\n)",
                                      lambda m: re.sub(r"@L(?:\d|[a-z]){1,23}",
                                                       lambda m2: m2.group(0) if m2.group(0)[1:] not in clients_users[
                                                           client.token] else "<@" + clients_users[client.token][
                                                           m2.group(0)[1:]] + ">", m.group(0)), text)
                        text = re.sub(r"-----@-----", "@", text)
                        if "subtype" in action and action["subtype"] == "file_share":
                            c.acquire()
                            file_info = {"ts": action["ts"], "name": action["file"]["name"], "id": action["file"]["id"],
                                         "child_ids": dict()}
                            file_is_old = False
                            ...
                            if not file_is_old:
                                ...
                                upload_response = json.loads(
                                    requests.post('https://slack.com/api/files.upload?' + urllib.parse.urlencode(get),
                                                  files=files).text)
